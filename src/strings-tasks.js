...
function isPalindrome(str) {
  const re = /[\W_]/g;
  const toRegStr = str.toLowerCase().replace(re, '');
  const reverseStr = toRegStr.split('').reverse().join('');
  return toRegStr === reverseStr;
}
...
function extractEmails(str) {
  ...
  const emails = emailArray.filter((email) => emailPattern.test(email.trim()));
